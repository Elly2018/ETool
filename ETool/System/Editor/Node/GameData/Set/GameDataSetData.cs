...
{
    [NodePath("Add Node/GameData/Set/SetData")]
    public class GameDataSetData : NodeBase
    {
        public GameDataSetData(Vector2 position, float width, float height) : base(position, width, height)
        {
            unlocalTitle = "Set Data";
        }
        ...
        {
            EGameData target = fields[1].target.genericUnityType.target_GameData;

            if(target != null)
            {
                string v1 = (string)GetFieldOrLastInputField(2, data);
                string v2 = (string)GetFieldOrLastInputField(3, data);
                object v3 = GetFieldOrLastInputField(5, data);
        ...
        private void UpdateFieldType()
        {
            ...
            {
                fields[5] = new Field(ty, "Value", ConnectionType.DataInput, this, FieldContainer.Object);
            }
        }
    }
}
