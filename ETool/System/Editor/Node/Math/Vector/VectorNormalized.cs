...
{
    [NodePath("Add Node/Math/Vector/VectorNormalized")]
    [Math_Menu("Vector")]
    public class VectorNormalized : NodeBase
    {
        public VectorNormalized(Vector2 position, float width, float height) : base(position, width, height)
        {
            unlocalTitle = "Vector Normalized";
        }
        ...
        private void TypeUpdate()
        {
            ...
            if (fields[1].fieldType != ft)
            {
                ...
                fields[1] = new Field(ft, "Result", ConnectionType.DataInput, this, FieldContainer.Object);
            }
        ...
        public object GetResultVector2(BlueprintInput data)
        {
            FieldType ft = (FieldType)fields[0].GetValue(FieldType.Vector);
            switch (ft)
            {
                case FieldType.Vector2:
                    return GetFieldOrLastInputField<Vector2>(2, data).normalized;
                case FieldType.Vector3:
                    return GetFieldOrLastInputField<Vector3>(2, data).normalized;
                case FieldType.Vector4:
                    return GetFieldOrLastInputField<Vector4>(2, data).normalized;
            }
            return GetFieldOrLastInputField<Vector2>(2, data).normalized;
        }
    }
