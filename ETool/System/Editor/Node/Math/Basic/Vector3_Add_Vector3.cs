...
namespace ETool.ANode
{
    ...
    public class Vector3_Add_Vector3 : NodeBase
    {
        public Vector3_Add_Vector3(Vector2 position, float width, float height) : base(position, width, height)
        {
        ...
        public override void FieldUpdate()
        {
            ...
            {
                change = false;
                if (fields.Count - 3 > (Int32)Field.GetObjectByFieldType(FieldType.Int, fields[1].target))
                {
                    change = true;
                    SubField();
                }
