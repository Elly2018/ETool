...
{
    [NodePath("Add Node/Math/Basic/SubtractVector")]
    public class MathSubVector : NodeBase
    {
        public MathSubVector(Vector2 position, float width, float height) : base(position, width, height)
        {
                ...
                case FieldType.Vector4:
                    {
                        ...
                        for (int i = 0; i < fields.Count - 3; i++)
                        {
                            answer -= (Vector4)GetFieldOrLastInputField(i + 3, data);
                        }
                        return answer;
                    }
            }
