...
{
    [NodePath("Add Node/Render/Material/Get/GetFloatArray")]
    public class MaterialGetFloatArray : NodeBase
    {
        public MaterialGetFloatArray(Vector2 position, float width, float height) : base(position, width, height)
        {
            unlocalTitle = "Get Material Float Array";
        }

        public override void FieldInitialize()
        {
            fields.Add(new Field(FieldType.Material, "Target", ConnectionType.DataBoth, true, this, FieldContainer.Object));
            fields.Add(new Field(FieldType.Float, "Result", ConnectionType.DataOutput, true, this, FieldContainer.Array));
        ...
        public float[] GetLightColor(BlueprintInput data)
        {
            if ((bool)fields[2].GetValue(FieldType.Boolean))
            {
                return GetFieldOrLastInputField<Material>(0, data).GetFloatArray(GetFieldOrLastInputField<int>(3, data));
            }
            else
            {
                return GetFieldOrLastInputField<Material>(0, data).GetFloatArray(GetFieldOrLastInputField<string>(3, data));
            }
        }
    }
}
