...
{
    [NodePath("Add Node/Render/Material/Get/GetTextureOffset")]
    public class MaterialGetTextureOffset : NodeBase
    {
        public MaterialGetTextureOffset(Vector2 position, float width, float height) : base(position, width, height)
        {
            unlocalTitle = "Get Material Texture Offset";
        }
        ...
        public Vector2 GetLightColor(BlueprintInput data)
        {
            if ((bool)fields[2].GetValue(FieldType.Boolean))
            {
                return GetFieldOrLastInputField<Material>(0, data).GetTextureOffset(GetFieldOrLastInputField<int>(3, data));
            }
            else
            {
                return GetFieldOrLastInputField<Material>(0, data).GetTextureOffset(GetFieldOrLastInputField<string>(3, data));
            }
        }
    }
}
